...
class GPSUpdater:
    ...
    def statusUpdater(self,statusIndex,message): #helper function used to accomplish the repetative task of updating the status array
        ...
        if(message=="NOT CONNECTED"):
            self.mylabels[statusIndex].config(fg="red")
        if(message=="DOWNLOADING"):
            self.mylabels[statusIndex].config(fg="orange")
        if(message=="WAITING FOR GPS"):
            self.mylabels[statusIndex].config(fg="orange")
        if(message=="READY TO REMOVE"):
            self.mylabels[statusIndex].config(fg="green")
        root.update()
    ...
    def scanPorts(self):
        ...
        self.ports.sort()
        #self.ports[0]="NOT A GPS" #hard code ports that arent supposed to be used.
        if(self.ports==[]): #keeps scanning for ports until one connects
            print("no devices connected, Waiting for device")
            #time.sleep(1) #used to slow down output for debugging
            self.scanPorts()
        for index, p in enumerate(self.ports):
            if 'USB' not in self.ports[index].description:
                self.ports[index]="NOT A SERIAL USB"
            ...
            statusReadout = Label(root, textvariable=self.GUIstatus[index],fg = "red")
            self.mylabels.insert(index,statusReadout)
            statusReadout.grid(row=(index+1),column=1) #status label creation uses the GUIstatus as text
    ...
    def handleUnit(self,port,statusIndex):  #Opens a single COM port and updates the status
        ...
        try:
            ...
            self.mylabels[statusIndex].config(fg="red") #default color
            root.update()
...
if __name__ == "__main__":
    ...
    while(True): 
        myUpdater.findUnits()
        root.update()
        #time.sleep(1) #makes debugging easier
